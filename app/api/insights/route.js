...
export async function POST(req) {
  try {
    ...
    insightsDatabase.unshift(newArticle);
    return NextResponse.json({ success: true, message: "تمت الإضافة بنجاح", data: newArticle });
  } catch (err) {
...
export async function PUT(req) {
  try {
    const body = await req.json();
    const index = insightsDatabase.findIndex(item => item.id === body.id);
    if (index !== -1) {
      insightsDatabase[index] = { ...insightsDatabase[index], ...body };
      return NextResponse.json({ success: true, message: "تم التعديل بنجاح", data: insightsDatabase[index] });
    }
...
export async function DELETE(req) {
  try {
    ...
    if (id) {
      insightsDatabase = insightsDatabase.filter(item => item.id !== id);
