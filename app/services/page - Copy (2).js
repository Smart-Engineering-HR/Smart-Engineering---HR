...
export default function ServicesPublicPage() {
  // القائمة الافتراضية الثابتة والآمنة لحين اكتمال الـ Hydration
  ...
  // تهيئة الحالة بالقيمة الافتراضية فوراً لمنع حدوث undefined
  ...
  // استدعاء البيانات بأمان بعد تحميل المكون في المتصفح فقط
  useEffect(() => {
    const savedData = localStorage.getItem('smart_engineering_services');
    if (savedData) {
      try {
        const parsed = JSON.parse(savedData);
        // التأكد من أن الكائن يحتوي على كافة القوائم المطلوبة قبل تحديث الستيت
        if (parsed && parsed.structural && parsed.architecture && parsed.smartTech && parsed.academy) {
    ...
    <div className="min-h-screen bg-[#050b14] text-slate-100 font-sans antialiased relative overflow-hidden bg-cover bg-center" style={{ backgroundImage: "linear-gradient(to bottom, rgba(5, 11, 20, 0.95), rgba(10, 25, 47, 0.92))" }}>
      ...
      <header className="border-b border-cyan-500/20 bg-slate-900/60 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center text-white font-bold text-xl tracking-wider">
              SE
            </div>
            <div>
          ...
          <button 
            ...
            className="group flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-950 border border-cyan-500/30 text-cyan-400 hover:text-white hover:bg-cyan-500/20 hover:border-cyan-400 transition-all duration-300"
          >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-lg text-slate-400 leading-relaxed">
            نقدم حزمة متكاملة من الخدمات الهندسية الاحترافية القائمة على الأتمتة البرمجية والتحليل الإنشائي الدقيق لرفع الكفاءة وضمان الجودة الصارمة.
          </p>
        </div>

        <div className="space-y-16">
          
          {/* 1. الخدمات الإنشائية والمدنية - مضاف إليها الحماية هنا وهناك لمنع أخطاء المتصفح */}
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              )) || <p className="text-xs text-slate-500">جاري تحميل الخدمات الإنشائية...</p>}
            </div>
          </section>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              )) || <p className="text-xs text-slate-500">جاري تحميل خدمات التصميم المعماري...</p>}
            </div>
          </section>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              )) || <p className="text-xs text-slate-500">جاري تحميل أدوات التحول الرقمي...</p>}
            </div>
          </section>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              )) || <p className="text-xs text-slate-500">جاري تحميل كورسات الأكاديمية...</p>}
            </div>
          </section>
        </div>
      </main>

      {/* مودال الطلبات الذكي المتكامل */}
      {selectedService && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0b1322] border border-cyan-500/30 rounded-3xl w-full max-w-3xl overflow-hidden shadow-[0_0_50px_rgba(6,182,212,0.25)] relative">
            ...
            <div className="p-6 bg-slate-900/80 border-b border-slate-800 flex justify-between items-start">
              ...
              <button onClick={() => setSelectedService(null)} className="p-1 rounded-lg text-slate-400 hover:text-white">✕</button>
            </div>
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              {notification && (
                <div className="mb-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex items-center gap-3 text-sm animate-pulse">
              ...
              {(activeTab === 'consultation' || activeTab === 'inquiry') && (
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea rows="4" required name="message" value={formData.message} onChange={handleInputChange} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-slate-200 focus:outline-none focus:border-cyan-400 text-sm resize-none" placeholder="يرجى كتابة كافة المتطلبات الهندسية الإضافية..."></textarea>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="border border-dashed border-slate-800 rounded-xl p-4 bg-slate-950/40 flex items-center justify-center gap-2 text-slate-400">
                      ...
                      <span className="text-xs">اضغط أو اسحب لرفع الملفات الفنية (PDF, CAD)</span>
                    </div>
                  </div>

                  <div className="pt-4 border-t border-slate-800 flex justify-end gap-3">
                    <button type="button" onClick={() => setSelectedService(null)} className="px-5 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 text-sm">إلغاء</button>
              ...
              {activeTab === 'contact' && (
                <div className="space-y-6 py-4">
                  <p className="text-sm text-slate-400 leading-relaxed">
                    قنوات الإيميل الرسمية ووسائل التواصل الفوري لمنصة الهندسة الذكية:
                  </p>
                  ...
                  <div className="space-y-3 pt-2">
                    <a href={secureSocialLinks.fbPlatform} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between bg-slate-950 p-3 rounded-xl border border-slate-800 hover:border-blue-500/40 transition-all group">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-slate-300">صفحة الهندسة الذكية على الفيس بوك</span>
                      </div>
      ...
      <footer className="border-t border-slate-900 mt-24 bg-slate-950/80 py-8 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-sm text-slate-500">
          <p>© 2026 منصة الهندسة الذكية. جميع الحقوق محفوظة لتطوير الأنظمة الهندسية.</p>
        </div>
      </footer>
    </div>
  );
