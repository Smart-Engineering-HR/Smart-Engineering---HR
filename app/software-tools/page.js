...
import { 
  ...
  HelpCircle, FileText, Upload, BarChart2, AlertTriangle, Layers, Grid, FileSpreadsheet, CheckCircle2, MessageSquare, RefreshCw, Calculator, Wrench, Play
} from "lucide-react";
...
export default function SoftwareToolsPublic() {
  ...
  // نموذج أطلب أداتك البرمجية الخاصة
  const [orderForm, setOrderForm] = useState({ name: "", email: "", phone: "", details: "" });
  const [isSubmittingOrder, setIsSubmittingOrder] = useState(false);
  const [orderSubmitted, setOrderSubmitted] = useState(false);

  // تفاعل مساعد المهندس الذكي AI
  const [aiSpecialization, setAiSpecialization] = useState("مدني");
  const [aiQuery, setAiQuery] = useState("");
  const [aiChatHistory, setAiChatHistory] = useState([]);
  const [isAiLoading, setIsAiLoading] = useState(false);

  // تفاعل محول الوحدات والملفات
  const [unitCategory, setUnitCategory] = useState("length");
  const [unitValue, setUnitValue] = useState(10);
  const [convertedResult, setConvertedResult] = useState(10);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased rtl relative" dir="rtl">
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Tab Content: Order Custom Tool Form */}
        {activeTab === "order-custom" && (
          <div className="max-w-2xl mx-auto bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl">
            ...
            <p className="text-slate-400 text-xs mb-6">سيتم إرسال المواصفات المطلوبة فورياً للإدارة والإيميلات الرسمية المعنيّة مع متابعتها باللوحة.</p>
            ...
            {orderSubmitted && (
              <div className="mb-6 p-4 bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 rounded-xl flex items-center gap-2 text-xs font-bold">
                ...
                <span>تم إرسال طلبك بنجاح وتحويل الإشعارات للبريد الإلكتروني للتحكم!</span>
              </div>
            )}
        ...
        {/* Tab Content: Quick Calculators F */}
        {activeTab === "quick-calculators" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { title: "🏗️ الهندسة المدنية", items: ["حاسبة كميات الخرسانة", "حاسبة حديد التسليح", "حاسبة الأحمال والكمرات", "حاسبة الأساسات والأعمدة", "حاسبة طرق ومساحة"] },
        ...
        {/* Tab Content: Smart AI Engineer G */}
          ...
          <div className="max-w-3xl mx-auto bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">استجابة هندسية وتحليل المخططات واستخراج الكميات</p>
                </div>
              </div>
              ...
              >
                <option value="🤖 مساعد مهندس مدني">🤖 مهندس مدني</option>
                <option value="🤖 مساعد معماري">🤖 مهندس معماري</option>
                <option value="🤖 مساعد كهربائي">🤖 مهندس كهربائي</option>
                <option value="🤖 مساعد ميكانيكي">🤖 مهندس ميكانيكي</option>
                <option value="🤖 مساعد إدارة مشاريع">🤖 إدارة مشاريع</option>
            ...
            <div className="min-h-[250px] max-h-[350px] overflow-y-auto space-y-3 p-3 bg-slate-950 rounded-2xl border border-slate-850 mb-4">
              {aiChatHistory.length === 0 ? (
                <div className="text-center text-slate-500 text-xs py-12">مثال: "أريد حساب كمية الخرسانة لبلاطة مساحتها 120 م² وسماكتها 15 سم"</div>
              ) : (
            ...
            <form onSubmit={handleAiAsk} className="flex gap-2">
              <input 
                type="text" placeholder="اسأل المهندس الذكي عن أي حسابات أو مخططات..." 
        ...
        {/* Tab Content: File & Unit Converter H */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-850">
                ...
                <div className="border-2 border-dashed border-slate-800 p-6 text-center rounded-xl hover:border-emerald-500/50 cursor-pointer">
                  ...
                  <span className="text-xs text-slate-400">اختر ملف DWG أو PDF للتحويل التلقائي</span>
                </div>
              </div>
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-850 space-y-3">
                ...
                <select value={unitCategory} onChange={e => setUnitCategory(e.target.value)} className="w-full bg-slate-900 border border-slate-800 text-xs text-white p-2.5 rounded-xl">
                  ...
                  <option value="area">المساحة (م² / هكتار / قدم²)</option>
        ...
        {/* Dynamic Cards Display for A, B, C, D, E */}
        {activeTab !== "order-custom" && activeTab !== "ai-engineer" && activeTab !== "file-converter" && activeTab !== "quick-calculators" && (
          isLoading ? (
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 rtl" dir="rtl">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-lg space-y-4">
              ...
              <div className="space-y-3 bg-slate-950 p-4 rounded-2xl border border-slate-850">
                ...
                <input 
                  type="number" placeholder="المساحة بالـ م² (input_area)" value={promptValues.input_area}
                ...
                />
                <input 
                  type="number" placeholder="الميزانية التقديرية (input_price)" value={promptValues.input_price}
